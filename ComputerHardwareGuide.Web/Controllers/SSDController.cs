...
namespace ComputerHardwareGuide.Web.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class SSDController : BaseComputerHardwareGuideController
    {
        public SSDController(IAppContext appContext) : base(appContext) { }

        [HttpGet]
        public async Task<IEnumerable<ComponentSSD>> Get(
            double? minPrice,
            double? maxPrice,
        ...
        {
            var query = AppContext.ComponentSSDs.Include(x => x.Firm)
